...
from backend.app.schemas.contracts import ScenarioPayload
...
class AnalysisService:
    ...
    def payoffs(self, scenario_payload: ScenarioPayload) -> dict[str, Any]:
        try:
            return build_bimatrix(payload_to_domain(scenario_payload))
    ...
    def arbitration(self, scenario_payload: ScenarioPayload, disagreement: tuple[float, float]) -> dict[str, Any]:
        try:
    ...
    def repeated(self, fixture_id: str, strategies: tuple[str, str], rounds: int, seed: int) -> dict[str, Any]:
        ...
        try:
            ...
            message = str(error)
            code = "INVALID_ROUNDS" if "round" in message.lower() else "UNSUPPORTED_REPEATED_STRATEGY"
            raise ApiError(code, message, "player_strategies", "Use a documented strategy and positive round count.") from error
    ...
    def full_analysis(self, scenario_payload: ScenarioPayload, include_repeated: bool, repeated_settings: Any | None) -> tuple[dict[str, Any], str]:
        ...
        final = {
            "outcome_id": matrix["pure_nash_equilibria"][0] if matrix["pure_nash_equilibria"] else None,
        ...
        data = {
            ...
            "prisoners_dilemma": {**pd, "fixture_type": "ELECTRICITY_ALLOCATION_GAME"},
            "uncertainty_analysis": uncertainty,
            "arbitration_result": self._full_arbitration_shape(arbitration),
